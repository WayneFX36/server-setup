...
Перед запуском скрипт показывает статус IP-фильтрации:
- Если настроена — показывает список разрешённых IP
- Если не настроена — предупреждает что будет применён стандартный IP `1.1.1.1`, и напоминает настроить в пункте [3]

### [3] Настройки — IP-фильтрация

Управление списком IP, которым разрешён доступ к порту ноды:

```
  [1]  Добавить IP (один или несколько через запятую)
  [2]  Удалить конкретный IP
  [3]  Удалить все IP и сбросить правила
  [4]  Применить текущий список к файрволу
```

Список хранится в `/etc/remnanode-allowed-ips.conf`. Правила применяются через UFW (Ubuntu) или firewalld rich rules (Rocky/RHEL). Статус фильтрации виден прямо в главном меню.
