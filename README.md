...
Запускает всё в правильном порядке. Перед стартом собирает все параметры сразу — больше не нужно вводить их по одному на каждом шаге:

```
  Установить Self SNI? (y/n)
  IP панели управления:
  Порт ноды (Enter для 2222):
  Secret Key ноды:
  → Подтверждение → установка
```

Затем запускает шаги без лишних вопросов:
...
- **Продолжить** — пропустит уже выполненные шаги, параметры восстанавливаются автоматически
- **Начать заново** — полная переустановка

IP панели можно указать сразу при установке или настроить позже в пункте **[3] Настройки**.
